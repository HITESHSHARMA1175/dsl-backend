        ...
        <div class="main-container container-fluid">
            <div class="inner-body">
                ...
                <div class="page-header">
                    <div>
                        <h2 class="main-content-title tx-24 mg-b-5">Edit Variant</h2>
                        <ol class="breadcrumb">
                ...
                <!-- End Page Header -->
                <form action="{{ route('variants.update', $variant_info->id) }}" method="post"
                                        ...
                                        <div class="">
                                            <div class="row row-sm">
                                                <div class="col-lg-6 form-group">
                                                    ...
                                                    onchange="getModelByBrandId(this.value,null)" required>
                                                        ...
                                                        @foreach ($brands as $item)
                                                            <option value="{{ $item->id }}" {{ $variant_info->brand == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-lg-6 form-group">
                                                    ...
                                                    <select class="form-control select select2" name="model" id="model" required>
                                                        <option value="">Select</option>
                                                        
                                                    </select>
                                                </div>
                                                <div class="col-lg-12 form-group">
                                                    <label class="form-label">Enter Variant Name: <span class="tx-danger">*</span></label>
                                                    <input class="form-control" name="name" value="{{ $variant_info->name }}" placeholder="Enter Variant Name" type="text" required> 
                                                </div>
...
<script>
    $(document).ready(function() {
        getModelByBrandId({{ $variant_info->brand }}, {{ $variant_info->model }});
    });
</script>
<script>
