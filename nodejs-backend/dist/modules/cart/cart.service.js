...
class CartService {
    constructor(prisma) {
        this.prisma = prisma;
        this.mailer = new nodemailer_service_1.NodemailerService();
    }
    ...
    async add(sessionKey, ipAddress, data) {
    ...
    async updateQty(id, qty) {
        ...
        if (!existing) {
            throw new appError_1.AppError(404, 'Cart item not found');
        }
        if (qty <= 0) {
     ...
     */
    async checkout(sessionKey, billing) {
