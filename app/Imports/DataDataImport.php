...
class DataDataImport implements ToCollection, WithHeadingRow
{
    private $staff_id;
    ...
    public function collection(Collection $row)
    {
        ...
        foreach ($rows as $key => $data) {
            ...
            @$import_id_last1 = Data::where('mobile_no',$data['mobile'])->orderBy('id', 'DESC')->pluck('id')->first();
            if(@$import_id_last1!=''){
                $duplicate_flag='1';
            ...
            if($data['mobile']!=''){
                if($key==0){
                    ...
                    $saveImportLog = new DataImportLog();
                    ...
                    $saveImportLog = DataImportLog::findOrFail($import_id_last);
                    $saveImportLog->duplicate = $duplicate;
                    $saveImportLog->updated = 0;
                    $saveImportLog->total_new = $assignNum;
                    $saveImportLog->save();
            ...
            if(@$duplicate_flag=='1'){
                $addSalesdataError = DataError::create([
